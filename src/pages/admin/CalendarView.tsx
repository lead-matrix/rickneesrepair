      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-8 bg-white border border-slate-200 rounded-2xl p-6 shadow-xs">
          ...
          <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold font-heading text-slate-700 uppercase tracking-wide">
          ...
          <div className="grid grid-cols-7 gap-2">
            {days.map((day, idx) => {
              if (!day) return <div key={`empty-${idx}`} className="aspect-square bg-slate-50/50 rounded-xl border border-transparent" />;
                  ...
                  <div className="flex justify-between items-center">
                    <span className={`text-xs font-bold ${
                      isToday 
                        ...
                        : isSelected ? 'text-accent font-extrabold' : 'text-slate-700'
                    }`}>
                      {day.getDate()}
                    </span>
              ...
              {selectedDateJobs.length > 0 ? (
                selectedDateJobs.map(job => {
                  ...
                  return (
                    <div 
                      key={job.id}
                      className="border border-slate-100 bg-slate-50/50 rounded-xl p-3.5 space-y-2 relative overflow-hidden"
                    >
                      <div className="absolute left-0 top-0 bottom-0 w-1 bg-accent" />
                      <div className="flex justify-between items-start">
                        <h4 className="font-bold text-slate-800 text-xs uppercase tracking-wide">{job.name}</h4>
                        <span className="text-[9px] font-extrabold bg-accent/15 text-accent px-2 py-0.5 rounded uppercase">
                          {job.id}
                        </span>
                      </div>
                      
                      <p className="text-xs font-semibold text-slate-600 uppercase">{job.brand} {job.applianceType}</p>
                      ...
                      <div className="space-y-1 text-[10px] text-slate-400">
                        ...
                        {tech && (
                          <div className="flex items-center gap-1.5 font-bold text-slate-500 uppercase tracking-wider pt-1.5 border-t border-slate-200/50">
